...
for nbs=1:N_bs
    ...
    tau0(:,nbs)=[0 Res_tau 1e-9*randperm(tau_idxRange,N_paths-2)+Res_tau]+norm(P_ue-P_bs(nbs,:))/c;
    
    theta_mp(:,nbs)=[gama(nbs)-gama_H0(nbs)+Res_aoa (randperm(theta_idxRange,N_paths-2)-(theta_idxRange/2))*Res_aoa];
    theta_mp(:,nbs)=theta_mp(randperm(N_paths-1),nbs);
    theta(:,nbs)=[gama(nbs)-gama_H0(nbs);theta_mp(:,nbs)];
    %                 theta/pi*180   
    ...
    Kvec=-2*pi/lamda*[(P_ue-P_bs(nbs,:))/norm(P_ue-P_bs(nbs,:));(cos(gama_H0(nbs)+theta(2:end,nbs))) (sin(gama_H0(nbs)+theta(2:end,nbs))) zeros(N_paths-1,1)];
    ...
    for k=1:N_ofdm
        ...
        for nmp=1:N_paths
            H0=H0+alpha(nmp)*A(:,nmp,nbs)*exp(-1j*2*pi*fn(k)*tau0(nmp,nbs));
        end
        Yt0=diag(exp(1j*2*pi*fix((k-1)/8)*CFO/delta_f))*B*H0*X(k,:);
...
if Qf~=1
    [MusicP_ue,~,~]=fminsearch(@(P_ue2)Music_dpd(Us,X(1:LenFreWindow),B,P_bs,P_ue2,D1,fn(1:LenFreWindow),lamda),P_ue(1:2));
    error3=norm([MusicP_ue,4]-P_ue);
end
[FastHRP_ue,~,~]=fminsearch(@(P_ue2)FastHR_dpd(Ydpd_t,X,B,P_bs,P_ue2,D1,fn,lamda),P_ue(1:2));
[MLP_ue,~,~]=fminsearch(@(P_ue2)MFdpd(Ydpd_t,X,B,P_bs,P_ue2,D1,fn,lamda),FastHRP_ue);
...
for kk1=1:length(xx)
    for kk2=1:length(yy)
        J_MLdpd(kk2,kk1)=-MFdpd(Ydpd_t,X,B,P_bs,[xx(kk1),yy(kk2)],D1,fn,lamda);
        J_FastHRdpd(kk2,kk1)=-FastHR_dpd(Ydpd_t,X,B,P_bs,[xx(kk1),yy(kk2)],D1,fn,lamda);
    end
end
...
if  Qf~=1
    ...
    for kk1=1:length(xx)
        for kk2=1:length(yy)
            J_Musicdpd(kk2,kk1)=-Music_dpd(Us,X(1:LenFreWindow),B,P_bs,[xx(kk1),yy(kk2)],D1,fn(1:LenFreWindow),lamda);
        end
    end
