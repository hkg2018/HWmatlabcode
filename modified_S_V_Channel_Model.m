...
[hN,N_h] = uwb_sv_cnvrt_ct_15_4a( alpha0, tau0, N_mp0, num_channels, Ts*1e9 );
if N_h > 1
    h = resample(hN, 1, N_h); % decimate the columns of hN by factor N
else
    h = hN;
end
...
if (cm_num == 1||cm_num == 2|| cm_num == 7||cm_num == 8 || cm_num ==9)
    [h]= uwb_sv_freq_depend_ct_15_4a(h,fc/1e9,fs/1e9,num_channels,kappa);
else
    [h]= uwb_sv_freq_depend_ct_15_4a(h,fc/1e9,fs/1e9,num_channels,0);
end
