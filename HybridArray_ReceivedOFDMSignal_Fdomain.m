...
yy=0.5:0.5:9.5;%场景网格
N_simu=10;
Ydpd_t=zeros(N_ofdm*M,Q,N_bs,N_simu,length(xx),length(yy));%通道输出频域数据
tau=zeros(N_mp,N_bs,N_simu,length(xx),length(yy));%多径信号时延
theta=zeros(N_mp,N_bs,N_simu,length(xx),length(yy));%多径相对方位角
for kk1=1:length(xx)
    for kk2=1:length(yy)
        %%%%%%%%%%%%%%%%%%%%%%%%%信号生成%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        parfor simu_idx=1:N_simu
            ...
            Ydpd_t0=zeros(N_ofdm*M,Q,N_bs);
            tau_idx=zeros(N_mp,N_bs);
            tau0=zeros(N_mp,N_bs);%多径信号时延
            theta_idx=zeros(N_mp-1,N_bs);
            theta0=zeros(N_mp,N_bs);%多径相对方位角
            ...
            for nbs=1:N_bs
                ...
                theta_idx(:,nbs)=[1  -1*randperm(fix((theta_idxRange-1)/2),fix(N_mp/2-1)) randperm(fix((theta_idxRange-1)/2)-1,ceil(N_mp/2-1))+1];
                theta_idx(:,nbs)=theta_idx(randperm(N_mp-1),nbs);
                theta0(:,nbs)=[gama0(nbs)-gama_H(nbs);theta_idx(:,nbs)*Res_aoa];
                ...
                Kvec=-2*pi/lamda*[(P_ue-P_bs0(nbs,:))/norm(P_ue-P_bs0(nbs,:));(cos(gama_H(nbs)+theta0(2:end,nbs))) (sin(gama_H(nbs)+theta0(2:end,nbs))) zeros(N_mp-1,1)];
                A(:,:,nbs)=exp(1j*(Kvec*D1(:,:,nbs)).');
                ...
                for k=1:N_ofdm
                    m_t0=zeros(M,Q);
                    for nmp=1:N_mp
                        m_t0=m_t0+B*alpha(nmp)*A(:,nmp,nbs)*X0(k,:)*exp(-1j*2*pi*fn0(k)*tau0(nmp,nbs));
                    end
                    Ydpd_t0((k-1)*M+1:k*M,:,nbs)=m_t0;
                end
                Ydpd_t0(:,:,nbs)=Ydpd_t0(:,:,nbs)+wgn(N_ofdm*M,Q,sigma2,'linear','complex');
            end
            Ydpd_t(:,:,:,simu_idx,kk1,kk2)=Ydpd_t0;
            tau(:,:,simu_idx,kk1,kk2)=tau0;
            theta(:,:,simu_idx,kk1,kk2)=theta0;
        end
    end
end
...
save(['..\mat\' 'DataSet(Frequency domain)' num2str(Res_aoa/pi*180) 'deg_' num2str(Res_tau*1e9) 'ns_' 'Npaths=' num2str(N_mp) 'N_simu=' num2str(N_simu) '.mat'],...
    'Ydpd_t','tau','theta', 'N_v','N_h','N_fft','delta_f','X','B','P_bs','D','fn','lamda');
